 ...
 * v1.1
 * 
 * Last Modified: 13 Sep 2021 (18:58 UTC+5)
 * 
...
function nik_pets_read64bit(bytearr, offset) {
	...
	let n2 = BigInt(nik_pets_read32bit(bytearr, oo + oint));
	return BigInt(n1 | (n2 << o32));
}
...
function nik_pets_write64bit(bytearr, offset, vv) {
	...
	nik_pets_write32bit(bytearr, oo + olong, n2);
}
...
function nik_pets_PatchTimebomb(bytearr) {
	...
	// generation begin.
	let v_firstRandom = (BigInt(Math.floor(Math.random() * v_intmax)) << BigInt(32)) | BigInt(Math.floor(Math.random() * v_intmax));
	...
	if (v_infoLocation < 0) v_infoLocation = -v_infoLocation;
	v_infoLocation %= BigInt(4);
	v_infoLocation = Number(v_infoLocation);
	...
	for (let i = 0; i < 4; ++i) {
		...
		let v_rfirst = BigInt(Math.floor(Math.random() * v_intmax));
		let v_rsecond = BigInt(Math.floor(Math.random() * v_intmax));
		let v_rcombined = BigInt( (v_rsecond >> BigInt(32)) | (v_rfirst & BigInt("0xffffffff")) );
		// write v_rcombined
		v_ids[1 + i] = v_rcombined;
	}
	
	// and now we need to write these values, god bless us.
	let j = gms2header_offs;
	for (let i = 0; i < 5; ++i) {
...
function nik_pets_FS_createDataFile(parent, name, data, canRead, canWrite, canOwn) {
	...
	if (Module && parent == "/assets/game.unx" && data) {
...
console.log("nik_pets_fucktimebomb: xhr hook ok?");
